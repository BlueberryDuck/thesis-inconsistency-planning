...
\begin{frame}[noframenumbering]{Backup, enriching the encoding still scores $1$}
    ...
    \begin{exampleblock}{The Rover dish toggle under two encodings}
        \footnotesize
        Precondition-only \quad $K_{\text{dish}} = \{\text{aimed\_earth},\ \text{aimed\_probe},\ \text{aimed\_probe} \rightarrow \text{aimed\_earth},\ \text{aimed\_earth} \rightarrow \text{aimed\_probe}\}$\\[2pt]
    ...
    \begin{block}{Detection without information}
        \begin{itemize}
            ...
            \item $B$ absorbs the conflict at one proposition, the same flat score as the drill chain
        \end{itemize}
    \end{block}
...
\begin{frame}[noframenumbering]{Backup, one problem, three structural categories}
    ...
    \begin{exampleblock}{The Rover Mission carries one failure per cluster}
        $G = \{\text{soil\_sampled}, \text{aimed\_earth}, \text{aimed\_probe}, \text{in\_orbit}, \text{mapped}\}$, three independent subsystems each contributing a structurally different failure.
    \end{exampleblock}
...
\begin{frame}[noframenumbering]{Backup, Bank Vault deepens the unreachability cascade}
    ...
    \begin{block}{The drill chain is the shallow case, Bank Vault the deep one}
        \begin{itemize}
            \item Drill chain, missing root \textit{drill\_bit} blocks \textit{hole\_drilled} then \textit{soil\_sampled}
            \item Bank Vault, three producerless roots feed three chains into \textit{in\_vault}
            \item Same scope, structure grows with chain count and length
        \end{itemize}
    \end{block}
...
\begin{frame}[noframenumbering]{Backup, symmetric sequencing raises the structure count}
    ...
    \begin{exampleblock}{Rival Alliances, both orders fail}
        $G = \{\text{allied\_A}, \text{allied\_B}\}$, each \textit{ally} operator deletes the rival's neutral precondition irreversibly, so the two goals never coexist.
    \end{exampleblock}

    \begin{block}{Structure value separates symmetric from one-directional}
        \begin{itemize}
            \item Rover orbit chain, one-directional, only $(\text{in\_orbit}, \text{mapped})$ fails, $\mathcal{I}_{\text{GS}} = (2, 1)$
            \item Rival Alliances, both $(\text{allied\_A}, \text{allied\_B})$ and $(\text{allied\_B}, \text{allied\_A})$ fail, $\mathcal{I}_{\text{GS}} = (2, 2)$
            \item Same scope of two goals, the structure count distinguishes one blocked order from two
