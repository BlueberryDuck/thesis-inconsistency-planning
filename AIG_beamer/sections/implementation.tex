...
\begin{frame}{Witness absence under brave reasoning is a universal proof}
    ...
    \begin{block}{Brave gives the right semantics}
        \begin{itemize}
            \item $\text{BRAVE}(\Pi)$ unions atoms across all answer sets, one per trace
            \item Witness $\in \text{BRAVE}$ means at least one trace satisfies the condition
            \item Witness $\notin \text{BRAVE}$ means no trace does, hence mutex or sequencing
            \item Cautious would intersect traces and give the wrong direction
        \end{itemize}
    \end{block}
