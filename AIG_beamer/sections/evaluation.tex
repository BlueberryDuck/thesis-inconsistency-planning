...
\begin{frame}{Selected case studies}
    \begin{exampleblock}{Scope grades severity}
        \textit{diagnosis} prob06 has 1 of 7 goals unreachable, prob05 49 of 56. The scope value $\mathcal{I}^{\text{scope}}_{\text{UR}}$ counts the unreachable goals, separating a mild defect from a near-total one.
    \end{exampleblock}

    \begin{exampleblock}{Structure grades density}
        \textit{diagnosis} prob10 has a single mutex pair, prob16 a full clique on 8 goals. The structure value $\mathcal{I}^{\text{struct}}_{\text{MX}}$ counts conflicting pairs, 1 against the upper bound $\binom{8}{2} = 28$, separating an isolated conflict from a dense one.
    \end{exampleblock}
