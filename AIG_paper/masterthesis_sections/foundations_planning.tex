...
\begin{example}[Blocksworld Domain]
    ...
    Consider a simplified blocksworld domain with predicates $\textit{on}(x,y)$, $\textit{ontable}(x)$, $\textit{clear}(x)$, and $\textit{handempty}$. The action for stacking block $x$ on block $y$ is defined as:
    \begin{align*}
         & \text{action } \textit{stack}(x, y)                                                 \\
         & \quad \text{precond}: \textit{holding}(x) \land \textit{clear}(y)                   \\
         & \quad \text{eff}: \textit{on}(x,y) \land \textit{clear}(x) \land \textit{handempty} \\
         & \qquad\qquad \land \neg\textit{holding}(x) \land \neg\textit{clear}(y)
    \end{align*}

    When instantiated with specific blocks $a$ and $b$ from a problem file, this yields the ground operator:
    \begin{align*}
        \textit{stack}(a,b) = \langle & \{\textit{holding}(a), \textit{clear}(b)\},                  \\
                                      & \{\textit{on}(a,b), \textit{clear}(a), \textit{handempty}\}, \\
                                      & \{\textit{holding}(a), \textit{clear}(b)\} \rangle
    \end{align*}
