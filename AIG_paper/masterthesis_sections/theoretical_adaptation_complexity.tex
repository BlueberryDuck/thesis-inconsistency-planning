...
This section analyzes the computational complexity of the proposed measures. All measures rely on forward reachability analysis as a fundamental building block.
...
\begin{prop}[Unreachability Profile Complexity]
    ...
    Given $S_{\text{reach}}$, computing $\mathcal{I}^{\text{scope}}_{\text{UR}}$ and
    $\mathcal{I}^{\text{struct}}_{\text{UR}}$ requires $O(|S_{\text{reach}}| \cdot |P|)$
...
\begin{prop}[Goal Mutex Profile Complexity]
    ...
    Given $S_{\text{reach}}$, computing $\mathcal{I}^{\text{scope}}_{\text{MX}}$ and
    $\mathcal{I}^{\text{struct}}_{\text{MX}}$ requires $O(|S_{\text{reach}}| \cdot |G|^2)$
...
\begin{prop}[Goal Sequencing Conflict Profile Complexity]
    ...
    Given $S_{\text{reach}}$, computing $\mathcal{I}^{\text{scope}}_{\text{GS}}$ and
    $\mathcal{I}^{\text{struct}}_{\text{GS}}$ requires $O(|G|^2 \cdot |S_{\text{reach}}|^2 \cdot |O|)$
    time and $O(|S_{\text{reach}}|)$ additional space.
\end{prop}
\begin{proof}
    Since $\text{REACH}(s) \subseteq S_{\text{reach}}$ for any $s \in S_{\text{reach}}$,
    local reachability is computed as graph traversal within the pre-computed state
    space rather than full reachability. For each goal pair $(g_1, g_2)$, check all
    $g_1$-states; each requires traversing at most $|S_{\text{reach}}|$ states with
    $|O|$ transitions each. There are $O(|G|^2)$ pairs and up to $|S_{\text{reach}}|$
    starting states per pair.
\end{proof}
...
The decision problems underlying each measure relate to known complexity classes. Computing $S_{\text{reach}}$ is closely related to plan existence, which is PSPACE-complete~\cite{Bylander1994}. Given $S_{\text{reach}}$, the individual measure computations are polynomial in the size of the reachable state set, as shown in Table~\ref{tab:complexity}. A detailed analysis of hardness and completeness results for the specific decision problems (e.g., ``is $\mathcal{I}^{\text{scope}}_{\text{UR}} \geq k$?'') is deferred to future work.
