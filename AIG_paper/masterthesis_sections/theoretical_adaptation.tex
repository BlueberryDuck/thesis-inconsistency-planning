...
The fourth proposal addresses unsolvability of Category~2c: sequencing conflicts where achieving certain goal propositions creates dead-end states that prevent other goal propositions from being reached. This category differs fundamentally from Category~1 goal conflicts. Here, goal propositions can coexist \emph{logically} but cannot both be \emph{achieved} due to irreversible action effects.

The \emph{goal sequencing conflict profile} characterizes action-based exclusions through reachability analysis from hypothetical goal states. The profile computes: (1) the number of goal propositions where achieving them makes other goal propositions unreachable or vice versa, and (2) the total number of ordered pairs $(g_1, g_2)$ where achieving $g_1$ makes $g_2$ unreachable. This parallels \ref{sec:goal_conflict_profile} in structure. The first value indicates scope, while the second reveals conflict patterns (star structures versus symmetric exclusions).
...
\paragraph{Scenario 2: Paint Contention.} A planning problem has two goal propositions each requiring 1 unit of paint, but only 1 unit is available (total need: 2, available: 1). The profile for paint:
\begin{itemize}
    \item Deficit: 1
    \item Consumers: 2 (two competing goal propositions)
\end{itemize}
