...
\end{prop}
\begin{proof}
    Counterexample: The Light Switch problem has $\mathcal{I}^{\text{scope}}_{\text{UR}}(\Pi) = 0$ (both goals \textit{on} and \textit{off} appear in reachable states at any $h \geq 1$) but is unsolvable (goals are mutex). Conversely, Planning Consistency requires $\mathcal{I} = 0$ only for solvable problems.
\end{proof}
...
\end{prop}
\begin{proof}
    Counterexample: The Locked Door problem has $\mathcal{I}^{\text{scope}}_{\text{MX}}(\Pi) = 0$ (no mutex relationships exist since the goal \textit{in\_room} is not even achievable) but is unsolvable.
\end{proof}
...
\end{prop}
\begin{proof}
    Counterexample: Consider $P = \{a, g_1, g_2\}$, $I = \{a\}$, $G = \{g_1, g_2\}$, and initial operator set $O = \{\text{reach\_g1}\}$ where $\text{reach\_g1} = \langle \{a\}, \{g_1\}, \{a\} \rangle$. Goal $g_1$ is achievable but $g_2$ is not, so no mutex exists and $\mathcal{I}^{\text{scope}}_{\text{MX}} = 0$. Adding operator $\text{reach\_g2} = \langle \{a\}, \{g_2\}, \{a\} \rangle$ makes $g_2$ achievable. However, no reachable state contains both $g_1$ and $g_2$, because both operators delete $a$, which is the only initial proposition. So $g_1$ and $g_2$ become mutex, increasing $\mathcal{I}^{\text{scope}}_{\text{MX}}$ to 2.
\end{proof}
...
\end{prop}
\begin{proof}
    Counterexample: The Light Switch problem has $\mathcal{I}^{\text{scope}}_{\text{GS}}(\Pi) = 0$ (no sequencing conflicts since actions are reversible) but is unsolvable (goals are mutex).
\end{proof}
