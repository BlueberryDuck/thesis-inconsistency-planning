...
\end{prop}
\begin{proof}
    Counter-example: The Light Switch problem has $\mathcal{I}^{\text{scope}}_{\text{UR}} = 0$ (both goals \textit{on} and \textit{off} appear in reachable states at any $h \geq 1$) but is unsolvable (goals are mutex). Conversely, Planning Consistency requires $\mathcal{I} = 0$ only for solvable problems.
\end{proof}
