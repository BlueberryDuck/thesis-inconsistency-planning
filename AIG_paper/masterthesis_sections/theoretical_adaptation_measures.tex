...
The set $S_{\text{reach}}$ can be computed via forward reachability analysis: starting from $I$, iteratively apply all applicable operators until no new states are generated. Since $P$ is finite and each state is a subset of $P$, this process terminates in at most $2^{|P|}$ iterations. Empirical convergence behavior on benchmark problems is discussed in Chapter~\ref{sec:evaluation}.
...
\begin{align*}
    ...
    \mathcal{I}^{\text{struct}}_{\text{UR}} & = |\{\text{key1}, \text{key2}, \text{admin\_appr},           \\
