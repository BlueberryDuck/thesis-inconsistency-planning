...
\begin{prop}[Monotonicity and Convergence of Bounded Reachability]
    ...
    Let $\Pi$ be a planning problem. Then:
    \begin{enumerate}
...
The Goal Mutex Profile addresses unsolvability where goal propositions are each individually achievable but never true in any common reachable state. This differs from specification-level contradictions (goals containing explicit negation like $\{p, \neg p\}$, which do not exist in propositional \ac{STRIPS}) and from sequencing conflicts (where achieving one goal permanently blocks another). Mutex relationships are operator-induced. Actions enforce mutual exclusion through their effects, creating states where goals alternate but never coexist.
...
This measure reveals the structure of mutex relationships by counting two-ele\-ment subsets rather than ordered pairs, reflecting the symmetry of the mutex relation. The value ranges from $0$ to $\binom{|G|}{2}$. The ratio between $\mathcal{I}^{\text{scope}}_{\text{MX}}$ and $\mathcal{I}^{\text{struct}}_{\text{MX}}$ reveals conflict patterns: a small ratio suggests isolated mutex pairs, while a large ratio (approaching $\binom{|G|}{2}$) indicates dense mutex cliques. Together, $\mathcal{I}^{\text{scope}}_{\text{MX}}$ and $\mathcal{I}^{\text{struct}}_{\text{MX}}$ form the Goal Mutex Profile.

\paragraph{Example: Light Switch (Scenario~3).} For the Light Switch scenario, forward reachability analysis yields $S_{\text{reach}} = \{\{\text{off}\}, \{\text{on}\}\}$. Both goal propositions appear in reachable states, but no state contains both. The Goal Mutex Profile yields:
\begin{align*}
...
\begin{defn}[Conflicting Goal Propositions Count]
    ...
    \begin{equation*}
        \begin{aligned}
            \mathcal{I}^{\text{scope}}_{\text{GS}}(\Pi, h) = |\{g \in G \mid {} & \exists g' \in G \text{ such that } (g, g') \text{ or } (g', g) \\
                                                                                & \text{is a sequencing conflict at horizon } h\}|
        \end{aligned}
    \end{equation*}
\end{defn}
    ...
    \begin{equation*}
        \begin{aligned}
            \mathcal{I}^{\text{struct}}_{\text{GS}}(\Pi, h) = |\{(g_1, g_2) \mid {} & g_1, g_2 \in G, g_1 \neq g_2,                                \\
                                                                                    & (g_1, g_2) \text{ is a sequencing conflict at horizon } h\}|
        \end{aligned}
    \end{equation*}
\end{defn}
...
\paragraph{Example: Trust and Travel (Scenario~5).} For the Trust and Travel scenario, forward reachability yields states including
\begin{equation*}
    \{\text{at\_local}, \text{trust}, \text{partnership}\} \quad \text{and} \quad \{\text{at\_local}, \text{opportunity}\},
\end{equation*}
...
\paragraph{Example: Rival Alliances (Scenario~6).} For the Rival Alliances scenario, forward reachability yields
\begin{equation*}
