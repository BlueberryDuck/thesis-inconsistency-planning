...
\begin{defn}[Horizon-Bounded Reachable States]
    ...
    Let $\Pi = \langle P, O, I, G \rangle$ be a planning problem and $h \in \mathbb{N}$. The set of \emph{horizon-bounded reachable states} $S^h_{\text{reach}} \subseteq 2^P$ is the set of all states reachable from $I$ by applying at most $h$ operators in sequence. Formally, $S^h_{\text{reach}}$ is the intersection of all sets of states satisfying:
    \begin{enumerate}
        \item $I \in S^h_{\text{reach}}$
        \item If $s \in S^h_{\text{reach}}$, $o \in O$ with $\text{precond}(o) \subseteq s$, and $s$ is reachable from $I$ in fewer than $h$ steps, then $\text{RESULT}(s,o) \in S^h_{\text{reach}}$
    \end{enumerate}
\end{defn}

This parameter plays an analogous role to the trace length parameter $m$ in linear temporal logic on fixed traces ($\text{LTL}_{\text{ff}}$). In that setting, Corea et al.~\cite{Corea2024} define inconsistency measures as functions $\mathcal{I} \colon \mathbb{K} \times \mathbb{N}_0 \to \mathbb{R}^{\infty}_{\geq 0}$ where the second argument is the trace length, and Kuhlmann~\cite{Kuhlmann2025b} follows the same convention. The measures defined below adopt this approach and take the planning problem and the horizon as joint input.
...
\begin{proof}
    Monotonicity follows directly from the definition: every state reachable in at most $h$ steps is also reachable in at most $h+1$ steps. For convergence, $S^h_{\text{reach}}$ is a non-decreasing sequence of subsets of~$2^P$, which is finite. Hence the sequence stabilizes at some $h^* \leq 2^{|P|}$, at which point $S^{h^*}_{\text{reach}} = S_{\text{reach}}$.
\end{proof}
...
An important consequence of the horizon bound is that the mutex and sequencing measures exhibit non-monotone behavior in $h$. Increasing $h$ simultaneously makes new goal propositions reachable, which may introduce apparent conflicts among newly achievable goals, and provides new execution traces, which may resolve previously reported conflicts through longer witness paths. This contrasts with the unreachability measure, where $\mathcal{I}^{\text{scope}}_{\text{UR}}$ is monotonically non-increasing in $h$ as a direct consequence of the monotonicity of $S^h_{\text{reach}}$. The non-monotone behavior of the mutex and sequencing measures is demonstrated empirically in Section~\ref{sec:eval_horizon}.
