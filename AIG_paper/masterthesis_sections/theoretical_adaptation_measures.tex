...
This section presents the three diagnostic measures with formal definitions. Each measure is defined, followed by illustrative examples using the test scenarios.
...
\begin{defn}[Diagnostic Profile]
    ...
    A \emph{diagnostic profile} is a pair $(\mathcal{I}^{\text{scope}}, \mathcal{I}^{\text{struct}})$ of inconsistency measures providing complementary views of a single structural phenomenon: the scope measure reports goal-level severity (how many goals are affected), while the structure measure reports the underlying structural detail (how extensive the conflicts are).
\end{defn}

\begin{defn}[Reachable States]
    \label{defn:reachable_states}
    Let $\langle P, O, I, G \rangle$ be a planning problem. The set of \emph{reachable states} $S_{\text{reach}}$ is defined as the least fixpoint of the generating operator on $2^{2^P}$ (which is unique since the operator is monotone on the power set lattice), that is, the smallest set satisfying:
    \begin{enumerate}
    ...
    \end{enumerate}
    where $\text{RESULT}(s,o) = (s \setminus \text{delete}(o)) \cup \text{add}(o)$. Each state $s \subseteq P$ is a set of propositions, so $S_{\text{reach}} \subseteq 2^P$.
\end{defn}

The set $S_{\text{reach}}$ can be computed via forward reachability analysis: starting from $I$, iteratively apply all applicable operators until no new states are generated (a fixpoint is reached). Since $P$ is finite and each state is a subset of $P$, this process terminates in at most $2^{|P|}$ iterations. Empirical convergence behavior on benchmark problems is discussed in Chapter~\ref{sec:evaluation}.
...
\begin{defn}[Unreachable Goal Propositions Count]
    Let $\langle P, O, I, G \rangle$ be a planning problem with reachable states $S_{\text{reach}}$. The measure $\mathcal{I}^{\text{scope}}_{\text{UR}}$ counting goal propositions that are unreachable is defined as:
    \begin{equation*}
        \mathcal{I}^{\text{scope}}_{\text{UR}}(\langle P, O, I, G \rangle) = |\{g \in G \mid \nexists s \in S_{\text{reach}} : g \in s\}|
    \end{equation*}
\end{defn}
...
\begin{defn}[Total Unreachable Propositions Count]
    Let $\langle P, O, I, G \rangle$ be a planning problem with reachable states $S_{\text{reach}}$. The measure $\mathcal{I}^{\text{struct}}_{\text{UR}}$ counting all unreachable propositions is defined as:
    \begin{equation*}
        \mathcal{I}^{\text{struct}}_{\text{UR}}(\langle P, O, I, G \rangle) = |\{p \in P \mid \nexists s \in S_{\text{reach}} : p \in s\}|
    \end{equation*}
\end{defn}
...
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{UR}}  & = |\{\text{in\_room}\}| = 1                                         \\
    \mathcal{I}^{\text{struct}}_{\text{UR}} & = |\{\text{has\_key}, \text{door\_unlocked}, \text{in\_room}\}| = 3
\end{align*}
...
\paragraph{Example: Bank Vault (Scenario~2).} For the Bank Vault scenario, reachability analysis yields $S_{\text{reach}} = \{\{\text{at\_facility}\}\}$ since none of the root preconditions (\textit{key1}, \textit{key2}, \textit{admin\_approval}) are initially true or produced by any operator. The Unreachability Profile yields:
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{UR}}  & = |\{\text{in\_vault}\}| = 1                                          \\
    \mathcal{I}^{\text{struct}}_{\text{UR}} & = |\{\text{key1}, \text{key2}, \text{admin\_approval},                \\
...
\begin{defn}[Mutex Goals Count]
    Let $\langle P, O, I, G \rangle$ be a planning problem. The measure $\mathcal{I}^{\text{scope}}_{\text{MX}}$ counting goal propositions involved in mutex relationships is defined as:
    \begin{equation*}
        \mathcal{I}^{\text{scope}}_{\text{MX}}(\langle P, O, I, G \rangle) = |\{g \in G \mid \exists g' \in G, g \neq g' : \text{mutex}(g, g')\}|
    \end{equation*}
\end{defn}
...
\begin{defn}[Mutex Pair Count]
    Let $\langle P, O, I, G \rangle$ be a planning problem. The measure $\mathcal{I}^{\text{struct}}_{\text{MX}}$ counting unordered mutex goal pairs is defined as:
    \begin{equation*}
        \mathcal{I}^{\text{struct}}_{\text{MX}}(\langle P, O, I, G \rangle) = |\{\{g_1, g_2\} \mid g_1, g_2 \in G, g_1 \neq g_2, \text{mutex}(g_1, g_2)\}|
    \end{equation*}
\end{defn}

This measure reveals the structure of mutex relationships by counting unordered pairs. Since mutex is symmetric, unordered pairs $\{g_1, g_2\}$ (where $\{g_1, g_2\}$ denotes the unordered pair, i.e., $\{g_1, g_2\} = \{g_2, g_1\}$) are counted rather than ordered pairs. The value ranges from $0$ to $\binom{|G|}{2}$. The ratio between $\mathcal{I}^{\text{scope}}_{\text{MX}}$ and $\mathcal{I}^{\text{struct}}_{\text{MX}}$ reveals conflict patterns: a small ratio suggests isolated mutex pairs, while a large ratio (approaching $\binom{|G|}{2}$) indicates dense mutex cliques. Together, $\mathcal{I}^{\text{scope}}_{\text{MX}}$ and $\mathcal{I}^{\text{struct}}_{\text{MX}}$ form the Goal Mutex Profile.
...
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{MX}}  & = |\{\text{on}, \text{off}\}| = 2     \\
    \mathcal{I}^{\text{struct}}_{\text{MX}} & = |\{\{\text{on}, \text{off}\}\}| = 1
\end{align*}
...
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{MX}}  & = |\{\text{red}, \text{yellow}, \text{green}\}| = 3                                                      \\
    \mathcal{I}^{\text{struct}}_{\text{MX}} & = |\{\{\text{red}, \text{yellow}\}, \{\text{red}, \text{green}\}, \{\text{yellow}, \text{green}\}\}| = 3
\end{align*}
This demonstrates a complete mutex clique: all goal pairs are mutually exclusive, yielding $\mathcal{I}^{\text{struct}}_{\text{MX}} = \binom{3}{2} = 3$. The profile distinguishes this dense conflict structure from isolated mutex pairs.
...
Informally, $(g_1, g_2)$ is a sequencing conflict when both goals are individually achievable, but achieving $g_1$ creates a permanent dead-end from which $g_2$ can never be satisfied. Note that sequencing conflict is directional: $(g_1, g_2)$ being a sequencing conflict does not imply $(g_2, g_1)$ is. Each ordered pair is checked independently. The requirement that $g_2$ be achievable (condition 2) ensures the conflict is meaningful: it distinguishes ``$g_1$ blocks $g_2$'' from ``$g_2$ was never achievable anyway.''
...
\begin{defn}[Conflicting Goal Propositions Count]
    Let $\langle P, O, I, G \rangle$ be a planning problem. The measure $\mathcal{I}^{\text{scope}}_{\text{GS}}$ counting goal propositions involved in sequencing conflicts is defined as:
    \begin{equation*}
        \mathcal{I}^{\text{scope}}_{\text{GS}}(\langle P, O, I, G \rangle) = |\{g \in G \mid \exists g' \in G : (g, g') \text{ or } (g', g) \text{ is a sequencing conflict}\}|
    \end{equation*}
\end{defn}
...
\begin{defn}[Sequencing Conflict Pair Count]
    Let $\langle P, O, I, G \rangle$ be a planning problem. The measure $\mathcal{I}^{\text{struct}}_{\text{GS}}$ counting ordered sequencing conflict pairs is defined as:
    \begin{equation*}
        \mathcal{I}^{\text{struct}}_{\text{GS}}(\langle P, O, I, G \rangle) = |\{(g_1, g_2) \mid g_1, g_2 \in G, g_1 \neq g_2, (g_1, g_2) \text{ is a sequencing conflict}\}|
    \end{equation*}
\end{defn}
...
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{MX}}  & = |\{\text{partnership}, \text{opportunity}\}| = 2     \\
    \mathcal{I}^{\text{struct}}_{\text{MX}} & = |\{\{\text{partnership}, \text{opportunity}\}\}| = 1
\end{align*}

The Goal Sequencing Conflict Profile yields:
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{GS}}  & = |\{\text{partnership}, \text{opportunity}\}| = 2   \\
    \mathcal{I}^{\text{struct}}_{\text{GS}} & = |\{(\text{opportunity}, \text{partnership})\}| = 1
\end{align*}
...
The Goal Mutex Profile yields:
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{MX}}  & = |\{\text{allied\_A}, \text{allied\_B}\}| = 2     \\
    \mathcal{I}^{\text{struct}}_{\text{MX}} & = |\{\{\text{allied\_A}, \text{allied\_B}\}\}| = 1
\end{align*}

The Goal Sequencing Conflict Profile yields:
\begin{align*}
    \mathcal{I}^{\text{scope}}_{\text{GS}}  & = |\{\text{allied\_A}, \text{allied\_B}\}| = 2                                         \\
    \mathcal{I}^{\text{struct}}_{\text{GS}} & = |\{(\text{allied\_A}, \text{allied\_B}), (\text{allied\_B}, \text{allied\_A})\}| = 2
\end{align*}
