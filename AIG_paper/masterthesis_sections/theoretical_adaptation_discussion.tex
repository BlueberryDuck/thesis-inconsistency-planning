...
State space analysis (unreachability, mutex) determines what states exist, while trajectory analysis (sequencing) determines what paths exist between states. For irreversible problems, all three measures report non-zero values, with sequencing providing directional information about which goal achievements cause the blockage.
...
\begin{equation*}
    \text{Profile}(\Pi, h) = (\mathcal{I}^{\text{scope}}_{\text{UR}}(\Pi, h), \mathcal{I}^{\text{struct}}_{\text{UR}}(\Pi, h), \mathcal{I}^{\text{scope}}_{\text{MX}}(\Pi, h), \mathcal{I}^{\text{struct}}_{\text{MX}}(\Pi, h), \mathcal{I}^{\text{scope}}_{\text{GS}}(\Pi, h), \mathcal{I}^{\text{struct}}_{\text{GS}}(\Pi, h))
\end{equation*}
...
The sequencing structure value distinguishes unidirectional conflicts ($\mathcal{I}^{\text{struct}}_{\text{GS}}(\Pi) = 1$ for Trust and Travel) from bidirectional conflicts ($\mathcal{I}^{\text{struct}}_{\text{GS}}(\Pi) = 2$ for Rival Alliances), while the mutex structure value is identical ($\mathcal{I}^{\text{struct}}_{\text{MX}}(\Pi) = 1$) for both. This illustrates how trajectory analysis (sequencing) provides finer-grained diagnostic information beyond state space analysis (unreachability, mutex).
