...
\begin{defn}[Sequencing Conflict]
    Let $\langle P, O, I, G \rangle$ be a planning problem with reachable states $S_{\text{reach}}$. For goal propositions $g_1, g_2 \in G$, a \emph{sequencing conflict} $(g_1, g_2)$ exists if:
    \begin{enumerate}
        \item $\exists s_1 \in S_{\text{reach}} : g_1 \in s_1$ (goal $g_1$ is achievable), and
        \item $\forall s \in S_{\text{reach}}$ with $g_1 \in s$, there is no reachable state $s' \in S_{\text{reach}}$ with $g_2 \in s'$ reachable from $s$
    \end{enumerate}
\end{defn}

Informally, $(g_1, g_2)$ is a sequencing conflict when $g_1$ can be achieved, but doing so prevents $g_2$ from being achieved subsequently.
