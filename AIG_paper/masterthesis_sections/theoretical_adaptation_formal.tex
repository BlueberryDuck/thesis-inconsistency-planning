...
\begin{defn}[Sequencing Conflict]
    ...
    \begin{enumerate}
        \item $\exists s \in S_{\text{reach}} : g_1 \in s$ (goal $g_1$ is achievable from $I$)
...
Informally, $(g_1, g_2)$ is a sequencing conflict when $g_1$ can be achieved, but from every state where $g_1$ holds, no sequence of actions can achieve $g_2$.

\textbf{Remark on Irreversibility.} The definition captures \emph{permanent} blocking: once any $g_1$-satisfying state is reached (whether through action execution or because $g_1 \in I$), $g_2$ becomes unreachable. This includes the edge case where $g_1$ holds in the initial state: if $g_1 \in I$ and $g_2$ is unreachable from $I$, then $(g_1, g_2)$ is a sequencing conflict. The key semantic property is irreversibility: achieving $g_1$ creates a ``point of no return'' from which $g_2$ can never be satisfied. This contrasts with mutex relationships (Definition~\ref{defn:mutex}), where goals may alternate between states via reversible actions but simply cannot co-occur.
...
\paragraph{Example: One-Way Door.} Consider the planning problem $\langle P, O, I, G \rangle$ with:
\begin{align*}
    P & = \{\text{outside}, \text{inside}, \text{has\_item}, \text{other\_task\_done}\}                       \\
    G & = \{\text{has\_item}, \text{other\_task\_done}\}                                                      \\
    I & = \{\text{outside}\}                                                                                  \\
    O & = \{\text{enter}, \text{get\_item}, \text{do\_other\_task}\}                                          \\
      & \text{where:}                                                                                         \\
      & \quad \text{enter} = \langle \{\text{outside}\}, \{\text{inside}\}, \{\text{outside}\} \rangle        \\
      & \quad \text{get\_item} = \langle \{\text{inside}\}, \{\text{has\_item}\}, \{\} \rangle                \\
      & \quad \text{do\_other\_task} = \langle \{\text{outside}\}, \{\text{other\_task\_done}\}, \{\} \rangle
\end{align*}

After entering (one-way door deletes $\text{outside}$), the agent can achieve $\text{has\_item}$ but can never return $\text{outside}$ to achieve $\text{other\_task\_done}$. The Goal Sequencing Conflict Profile yields:
\begin{align*}
    I^{\text{scope}}_{\text{GS}}  & = |\{\text{has\_item}, \text{other\_task\_done}\}| = 2 \\
    I^{\text{struct}}_{\text{GS}} & = |{(\text{has\_item}, \text{other\_task\_done})}| = 1
\end{align*}

The profile detects one unidirectional conflict: achieving $\text{has\_item}$ blocks $\text{other\_task\_done}$.
...
Achieving either goal deletes the precondition needed for the other goal's achievement. The Goal Sequencing Conflict Profile yields:
\begin{align*}
    I^{\text{scope}}_{\text{GS}}  & = |\{\text{goal\_X}, \text{goal\_Y}\}| = 2                                   \\
    I^{\text{struct}}_{\text{GS}} & = |{(\text{goal\_X}, \text{goal\_Y}), (\text{goal\_Y}, \text{goal\_X})}| = 2
\end{align*}

This example demonstrates symmetric exclusions where both goals block each other, distinguishing them from unidirectional conflicts.
