...
A natural approach to measuring inconsistency in planning problems would encode the goal specification as a propositional knowledge base and apply classical inconsistency measures. Specifically, encoding the goal $G$ as $\bigwedge_{g \in G} g$ with appropriate mutex constraints should allow direct application of measures such as the contension measure $I_c$.
...
\begin{table}[ht]
    ...
    \begin{tabular}{llc}
        ...
        2c: Sequencing Conflict & One-Way Door    & 1     \\
        \hline
    \end{tabular}
\end{table}
...
\subsection{From Structural Categories to Measurement Profiles}

The structural taxonomy organizes unsolvability causes by their mathematical properties: goal specification conflicts (Category~1), dependency graph failures (Category~2), and resource insufficiency (Category~3). The measurement framework below focuses on Category~2 problems, organizing three measures into two complementary \emph{analysis profiles}.

The framework distinguishes between two types of reachability analysis. \textbf{State Space Analysis} (Profile~1) examines which states exist in the reachable state space, asking ``do the required states exist?'' This static analysis computes reachability once and analyzes the resulting state set. \textbf{Trajectory Analysis} (Profile~2) examines path-dependent properties, asking ``do valid paths exist between states?'' This dynamic analysis considers reachability from intermediate goal-achieving states.

Profile~1 contains two measures detecting different state-level failures: unreachability (propositions never appearing in any state) and mutex (goals never co-occurring in any state). Profile~2 contains one measure detecting trajectory-level failures: sequencing conflicts (achieving one goal creates dead-ends blocking others). Together, these three measures provide complementary diagnostic perspectives on Category~2 dependency problems.
...
The three proposals are organized into two analysis profiles:

\textbf{Profile 1: State Space Analysis (Static)} examines which states exist in the reachable state space. This profile contains two measures:
...
\textbf{Profile 2: Trajectory Analysis (Dynamic)} examines path-dependent properties of the state space:

\emph{Proposal 3: Goal Sequencing Conflict Profile} detects irreversible dead-end states where achieving certain goal propositions permanently blocks others. Unlike mutex (where goals can be achieved in different states), sequencing conflicts involve one-way transitions that prevent subsequent goal achievement. The profile reports which goal propositions participate in conflicts and counts ordered conflict pairs, revealing directionality.
...
This measure identifies conflicts from action effects. Goal propositions can theoretically coexist in the same state, but action sequences required to achieve one proposition prevent achieving the other, suggesting missing reversibility in the action model.
