...
However, this naive encoding yields uninformative results. Table~\ref{tab:naive_ic} shows that both the contension measure $I_c$ and the minimal inconsistent subset count $I_{\text{MI}}$ yield identical values for representative problems from each structural category.

\begin{table}[ht]
    \centering
    \caption{Classical inconsistency measures for naive propositional encodings}
    \label{tab:naive_ic}
    \begin{tabular}{llcc}
        \hline
        Category                & Example Problem  & $I_c$ & $I_{\text{MI}}$ \\
        \hline
        2a: Missing Producers   & Locked Door      & 1     & 1               \\
        2c: Mutex               & Light Switch     & 1     & 1               \\
        2c: Sequencing Conflict & Package Delivery & 1     & 1               \\
        \hline
    \end{tabular}
\end{table}

This uniform result fails to distinguish fundamentally different types of unsolvability. The limitation is not specific to individual measures: both syntax-based ($I_{\text{MI}}$) and semantics-based ($I_c$) approaches yield identical values because the propositional encoding reduces structurally different planning failures to equivalent logical conflicts. For $I_c$, mutual exclusivity constraints $\neg(p \land q)$ are satisfied by assigning $B$ to only one proposition. For $I_{\text{MI}}$, each encoding produces exactly one minimal inconsistent subset regardless of the underlying planning structure.
...
\paragraph{Scenario 1: Locked Door.} An agent must enter a room, but entry requires unlocking a door, which requires a key that no operator produces. The profile yields 1 unreachable goal but 3 total unreachable propositions, revealing a dependency chain of depth 2.

\paragraph{Scenario 2: Bank Vault.} A more complex variant requires three independent precondition chains (two keys and admin approval) to achieve a single goal. The profile yields 1 unreachable goal but 7 total unreachable propositions, revealing three parallel missing-producer failures.

A small difference between scope and structure indicates shallow failures; a large difference reveals deep cascading dependencies. Formal operator definitions for these scenarios appear in Section~\ref{sec:formalization}. This measure applies uniformly across all Category~2 subtypes, providing a generic diagnostic for dependency-related unsolvability.
...
\paragraph{Scenario 1: Light Switch.} A goal requires both \textit{on} and \textit{off} to be true, but toggle actions enforce mutual exclusion. Both propositions are individually achievable and actions are reversible, but no state contains both. The profile yields 2 mutex goals and 1 mutex pair.

\paragraph{Scenario 2: Traffic Light.} A goal requires all three colors (\textit{red}, \textit{yellow}, \textit{green}) simultaneously, but cyclic transitions enforce that exactly one color is active at any time. The profile yields 3 mutex goals and 3 mutex pairs (a complete clique).

This measure identifies operator-induced mutual exclusions where goals cannot coexist despite each being individually achievable. The ratio of mutex pairs to mutex goals distinguishes isolated conflicts from dense cliques. Formal operator definitions appear in Section~\ref{sec:formalization}.
...
\paragraph{Scenario 1: Package Delivery.} An agent must deliver a fragile package locally and complete a remote task, but transport destroys the package irreversibly. Both goals are individually achievable, but completing the remote task permanently blocks delivery. The profile yields 2 conflicting goals and 1 ordered conflict pair (\textit{remote\_done} blocks \textit{delivered}).

\paragraph{Scenario 2: Bidirectional Conflict.} Each of two goals, when achieved, permanently blocks the other. The profile yields 2 conflicting goals and 2 ordered conflict pairs (both directions), distinguishing this symmetric case from the unidirectional Package Delivery scenario.

This measure provides directional information beyond mutex. Sequencing conflicts are a subset of mutex relationships (blocking implies non-coexistence), but the ordered pair count reveals \emph{which} goal achievement causes the dead-end. Reversible mutex problems (Light Switch) exhibit mutex without sequencing; irreversible problems (Package Delivery) exhibit both. Formal operator definitions appear in Section~\ref{sec:formalization}.
