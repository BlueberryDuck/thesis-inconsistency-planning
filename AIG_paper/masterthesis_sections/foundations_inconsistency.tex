...
\cite{Ulbricht2016} adapted inconsistency measures to Answer Set Programming, addressing the fundamental challenge that non-monotonic reasoning violates classical monotonicity postulates. Their key observation---that adding information to a non-monotonic knowledge base can \emph{reduce} inconsistency---directly parallels the planning context, where adding operators can enable new solution paths. This motivates the Operator Monotonicity postulate developed in Section~\ref{sec:postulates}.

\cite{Corea2022} developed inconsistency measures for temporal logic applied to declarative process specifications. Their work demonstrates that traditional measures fail to adequately capture temporal operators, requiring domain-specific semantics for meaningful conflict quantification. This parallels our finding that naive propositional encodings of planning problems yield uninformative measure values, motivating planning-native measures.
